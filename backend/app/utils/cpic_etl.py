    ...
    def _process_allele_functionality(self, gene_symbol: str, file_path: Path):
        ...
        try:
            ...
            for _, row in df.iterrows():
                ...
                # Check for empty
                if allele_raw in ('nan', ''):
                    continue
                    
                # Standard CPIC: starts with *
                if allele_raw.startswith('*'):
                     allele = allele_raw.split()[0]
                # DPYD fallback (cDNA notation)
                elif gene_symbol == "DPYD" and (allele_raw.startswith('c.') or allele_raw.lower() == 'reference'):
                    if allele_raw.lower() == 'reference':
                        allele = "*1" # Map reference to wildcard
                    else:
                        # Extract alias (*2A) if present
                        # Format: "c.1905+1G>A (*2A)"
                        import re
                        match = re.search(r'\(\*(\S+)\)', allele_raw)
                        if match:
                            allele = "*" + match.group(1)
                        else:
                            # Use cDNA name if no star alias (unlikely for core alleles)
                            allele = allele_raw.split()[0]
                else:
                    # Skip other text
                    continue
    ...
    def _process_drug_alerts(self, gene_symbol: str, file_path: Path):
        ...
        try:
            ...
            for _, row in df.iterrows():
                ...
                # Prepare context early to check for No CDS
                context = str(row.get(context_col, '')).strip() if context_col else ''
                
                # Skip if no alert text AND not explicit "No CDS"
                # (No CDS rows often have empty alert text because no alert is needed)
                if alert_text in ('nan', '') and context != "No CDS":
                    continue
